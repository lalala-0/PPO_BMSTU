...
type ICrewService interface {
	AddNewCrew(crewID uuid.UUID, ratingID uuid.UUID, class string, sailNum int) (*models.Crew, error)
	DeleteCrewByID(id uuid.UUID) error
	UpdateCrewByID(crewID uuid.UUID, ratingID uuid.UUID, class string, sailNum int) (*models.Crew, error)
