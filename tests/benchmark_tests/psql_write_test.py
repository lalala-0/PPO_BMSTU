...
class PostgresBenchmark(User):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            events.request.fire(
                ...
                name="PostgreSQL ratings.insert",
                response_time=0,
                response_length=0,
                exception=e
            )
    ...
    def test_postgresql_insert(self):
        ...
        try:
            ...
            query = "INSERT INTO ratings (name, class, blowout_cnt) VALUES (%s, %s, %s)"
            values = ("name", 1, 1)
            ...
            events.request.fire(
                ...
                response_length=1,  # Длина ответа (1 строка)
                success=True
            )
            print(f"PostgreSQL insert time: {duration} seconds")
        except Exception as e:
            pg_errors.inc()
            events.request.fire(
                request_type="POSTGRESQL",
