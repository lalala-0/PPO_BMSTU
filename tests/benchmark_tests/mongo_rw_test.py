...
class MongoBenchmark(User):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            events.request.fire(
                ...
                name="MongoDB ratings.read_write",
                response_time=0,
                response_length=0,
                exception=e
            )
    ...
    @task
    def test_mongodb_read_write(self):
        ...
        try:
            ...
            # Чтение документа по имени
            inserted_document = self.mongo_collection.find().limit(1000)
            ...
            print(f"MongoDB read/write time: {duration} seconds, inserted document ID: {result.inserted_id}")
        except Exception as e:
            mongo_errors.inc()
            events.request.fire(
                request_type="MONGODB",
            ...
            print(f"MongoDB read/write error: {e}")
        finally:
