...
class PostgresBenchmark(User):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            events.request.fire(
                ...
                name="PostgreSQL ratings.read_write",
                response_time=0,
                response_length=0,
                exception=e
            )
    ...
    @task
    def test_postgresql_read_write(self):
        ...
        try:
            ...
            insert_query = "INSERT INTO ratings (name, class, blowout_cnt) VALUES (%s, %s, %s)"
            values = ("name", 1, 1)
            self.pg_cursor.execute(insert_query, values)
            self.pg_conn.commit()

            # Чтение только что вставленных данных
            self.pg_cursor.execute("SELECT * FROM ratings limit 1000")
            ...
            events.request.fire(
                ...
                response_length=len(result),  # Длина ответа (количество строк)
                success=True
            )
            print(f"PostgreSQL read/write time: {duration} seconds, rows returned: {len(result)}")
        except Exception as e:
            pg_errors.inc()
            events.request.fire(
                request_type="POSTGRESQL",
            ...
            print(f"PostgreSQL read/write error: {e}")
        finally:
